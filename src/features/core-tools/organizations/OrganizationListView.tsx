...
import { organizationListMock } from './data';

export function OrganizationListView() {
  const [keyword, setKeyword] = useState('');

  const filtered = organizationListMock.filter((item) => {
  ...
  return (
    <div className="rounded-md bg-white p-4 text-xs shadow-sm">
      ...
      <OrganizationTable
        data={filtered}
        onShare={(row) => console.log('share', row)}
        onUsers={(row) => console.log('users', row)}
