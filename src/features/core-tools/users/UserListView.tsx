...
export function UserListView() {
  ...
  const filtered = userListMock.filter((item: UserItem) => {
    ...
    return (
      ...
      item.roles.some((r) => r.toLowerCase().includes(searchLower))
    );
  });
  ...
  return (
    <div className="rounded-md bg-white p-4 text-xs shadow-sm">
      ...
      <UserTable
        ...
        onToggleActive={(row) => console.log('toggle active', row)}
      />
    </div>
  );
}
