import { useState } from 'react';
import { DataTable } from 'primereact/datatable';
import { Column } from 'primereact/column';
import { Button } from '@/components/ui/button';
import { Share2, Users, Pencil, Trash2 } from 'lucide-react';
import { EmptyTableState } from '@/components/ui/empty-table-state';
import type { OrganizationItem } from './data';

const EMPTY_MESSAGE = 'Không có tổ chức nào.';

type Props = {
  data: OrganizationItem[];
  loading?: boolean;
  onShare?: (row: OrganizationItem) => void;
  onUsers?: (row: OrganizationItem) => void;
  onEdit?: (row: OrganizationItem) => void;
  onDelete?: (row: OrganizationItem) => void;
};

export function OrganizationTable({
  data,
  loading,
  onShare,
  onUsers,
  onEdit,
  onDelete,
}: Props) {
  const [first, setFirst] = useState(0);
  const [rows, setRows] = useState(10);

  if (!loading && data.length === 0) {
    return (
      <div className="w-full overflow-auto">
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50">
              <th className="w-20 px-3 py-2 text-left font-semibold text-slate-700">STT</th>
              <th className="px-3 py-2 text-left font-semibold text-slate-700">Mã tổ chức</th>
              <th className="px-3 py-2 text-left font-semibold text-slate-700">Tên tổ chức</th>
              <th className="px-3 py-2 text-left font-semibold text-slate-700">Mô tả</th>
              <th className="px-3 py-2 text-left font-semibold text-slate-700">Mã quốc gia</th>
              <th className="px-3 py-2 text-left font-semibold text-slate-700">Điện thoại liên hệ</th>
              <th className="w-32 px-3 py-2 text-left font-semibold text-slate-700">Thao tác</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td colSpan={7} className="p-0 align-top">
                <EmptyTableState message={EMPTY_MESSAGE} />
              </td>
            </tr>
          </tbody>
        </table>
      </div>
    );
  }

  return (
    <DataTable
      value={data}
      loading={loading}
      paginator={data.length > 0}
      first={first}
      rows={rows}
      rowsPerPageOptions={[10, 20, 50]}
      onPage={(e) => {
        setFirst(e.first);
        setRows(e.rows);
      }}
      size="small"
      emptyMessage={EMPTY_MESSAGE}
      className="w-full text-xs"
      sortMode="multiple"
      currentPageReportTemplate="Hiển thị từ {first} đến {last} trong tổng số {totalRecords}"
      paginatorTemplate="CurrentPageReport FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink"
    >
      <Column
        header="STT"
        body={(_, options) => options.rowIndex + 1}
        style={{ width: '80px' }}
      />
      <Column field="code" header="Mã tổ chức" sortable />
      <Column
        field="name"
        header="Tên tổ chức"
        sortable
        body={(row: OrganizationItem) => (
          <span className="text-[#be8a4b]">{row.name}</span>
        )}
      />
      <Column
        field="description"
        header="Mô tả"
        body={(row: OrganizationItem) =>
          row.description ? (
            <span className="text-blue-600">{row.description}</span>
          ) : (
            <span className="text-slate-400">-</span>
          )
        }
      />
      <Column field="countryCode" header="Mã quốc gia" sortable />
      <Column field="phone" header="Điện thoại liên hệ" sortable />
      <Column
        header="Thao tác"
        body={(row: OrganizationItem) => (
          <div className="flex items-center gap-1">
            <Button
              variant="ghost"
              size="icon-xs"
              type="button"
              onClick={() => onShare?.(row)}
              aria-label="Chia sẻ"
            >
              <Share2 className="h-3.5 w-3.5 text-slate-400" />
            </Button>
            <Button
              variant="ghost"
              size="icon-xs"
              type="button"
              onClick={() => onUsers?.(row)}
              aria-label="Người dùng"
            >
              <Users className="h-3.5 w-3.5 text-slate-400" />
            </Button>
            <Button
              variant="ghost"
              size="icon-xs"
              type="button"
              onClick={() => onEdit?.(row)}
              aria-label="Sửa"
            >
              <Pencil className="h-3.5 w-3.5 text-[#c8872a]" />
            </Button>
            <Button
              variant="ghost"
              size="icon-xs"
              type="button"
              onClick={() => onDelete?.(row)}
              aria-label="Xóa"
            >
              <Trash2 className="h-3.5 w-3.5 text-slate-400" />
            </Button>
          </div>
        )}
        style={{ width: '140px' }}
      />
    </DataTable>
  );
}
