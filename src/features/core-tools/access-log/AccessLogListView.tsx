...
import { accessLogMock } from './data';
import type { AccessLogItem } from './data';
import { Input } from '@/components/ui/input';
import { Button } from '@/components/ui/button';
import { RefreshCcw, PanelTop, Settings2, Search, Download } from 'lucide-react';

export function AccessLogListView() {
  const [keyword, setKeyword] = useState('');

  const filtered = accessLogMock.filter((item: AccessLogItem) => {
  ...
  return (
    <div className="rounded-md bg-white p-4 text-xs shadow-sm">
      ...
      <AccessLogTable
        data={filtered}
        onViewDetail={(row) => console.log('Xem chi tiết', row)}
      />
    </div>
  );
}
